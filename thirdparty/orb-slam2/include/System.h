...
#include "SerializedSystem.pb.h"
#include "Tracking.h"
#include "Viewer.h"

namespace ORB_SLAM2 {

void LoadSystemProto(const std::string& systemFile,
    SerializedSystem* systemProto);
...
public:
  ...
  System(ORBVocabulary* vocabulary, const string &strSettingsFile,
      const eSensor sensor, const bool bUseViewer = true);
  System(const SerializedSystem& serialized, const string &strVocFile,
  ...
  // Input images: RGB (CV_8UC3) or grayscale (CV_8U). RGB is converted to grayscale.
  // Returns the camera pose (empty if tracking fails).
  cv::Mat TrackMonocular(const TimestampedImage& frame);
  ...
  // This is only safe to call after shutdown().
  SerializedSystem Serialize();

  const Tracking& tracker() const {
    return *mpTracker;
  }

  std::vector<PoseWithTimestamp> GetTrajectory();

private:

  void InitAll(const string& strSettingsFile, bool bUseViewer);

  void InitPreMap();

  void InitDrawers(const string& strSettingsFile);

  void InitPostMap(const string& strSettingsFile, bool bUseViewer);
  ...
  bool is_vocabulary_owned_ = false;
