...
Это пост-отчет-тьюториал про беспилотные автомобили - как (начать) делать свой.
...
![Sensors to direcrtions](01-steering/autopilot-sensors.jpg)

Простейший же вариант сенсора - видеокамера, "смотрящая" через лобовое стекло. С ним и будем работать.

![Lane from video](01-steering/lane-from-video.jpg)
...
нужно обучить. Для обучения нужно выбрать архитектуру модели и сформировать обучающую
выборку, которая будет демонстрировать модели различные входные ситуации и
"правильные ответы" (например, угол поворота руля и положение педали газа)
...
а вот с данными более печально: во-первых данных просто мало, во-вторых почти все выборки - 
...
нужную информацию - скорость движения и поворотов, на которых уже можно буде
обучать автопилот. В этой серии - "выжималка" угла поворота из видео. 
Все шаги легко повторить своими силами с помощью кода [на github](https://github.com/waiwnf/pilotguru).
...
поступательную скорость, которой мы займемся в следующей серии.
...
смещение и 3 [угла ориентации](https://ru.wikipedia.org/wiki/%D0%A3%D0%B3%D0%BB%D1%8B_%D0%AD%D0%B9%D0%BB%D0%B5%D1%80%D0%B0).
...
* Системе SLAM нужна "хорошая" последовательность кадров, за которую можно
  ...
  система не "зацепится" останется не аннотированным.
...
* Из сути главных компонент понятно, что кроме горных дорог выделение главной
  ...
  большой разброс значений, а разброс по оставшемуся перпендикулярному 
...
![Horizontal projection equation](01-steering/horizontal-projection-equation.gif)
...
![Horizontal projection equation](01-steering/rotation-magnitude.gif)
...
### Сглаживаем, чтобы убрать шумы

5. Мы почти у цели, но остается еще проблема. Посмотрим на получившийся (пока 
...
низкочастотным фильтром. Результат после фильтра:

[picture]

[video]

Всё! это уже можно "скормить" обучающемой модели и рассчитывать на адекватные
результаты.

В следующей серии - определяем текущую скорость, чтобы обучить еще и управление
скоростью, а пока что очень приветствуются pull-request'ы.
